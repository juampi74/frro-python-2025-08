...
def obtener_datos_nasa_mensual(lat, lon, años_atras=15):
    """
    Versión alternativa que obtiene datos mensuales (menos datos, más rápido)
    """
    fecha_actual = datetime.now()
    fecha_inicio = fecha_actual - timedelta(days=años_atras * 365)
    ...
    print(f"Obteniendo datos mensuales NASA POWER...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            fechas_dt = [datetime.strptime(fecha, '%Y%m') for fecha in fechas]
            
            df = pd.DataFrame({
                'fecha': fechas_dt,
                'temperatura_media_C': [parametros['T2M'][f] for f in fechas],
                'humedad_relativa_%': [parametros['RH2M'][f] for f in fechas],
                'velocidad_viento_m_s': [parametros['WS2M'][f] for f in fechas],
                'precipitacion_mm_mes': [parametros['PRECTOTCORR'][f] for f in fechas]
            })
...
if __name__ == "__main__":
    ...
    print("=== INTENTANDO DATOS DIARIOS ===")
    # Intentar primero con datos diarios
    df_clima = obtener_datos_nasa_power(latitud, longitud, años_atras=15)
    
    # Si falla, intentar con datos mensuales
    if df_clima.empty:
        print("\n=== INTENTANDO DATOS MENSUALES ===")
        df_clima = obtener_datos_nasa_mensual(latitud, longitud, años_atras=15)
    
    if not df_clima.empty:
        # Guardar en CSV
        output_path = "clima_nasa_15_anios.csv"
